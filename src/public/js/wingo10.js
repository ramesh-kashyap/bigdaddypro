function showResultPopup(result, stage, money, join, resultwingo) {
  ...
  let popupOutcome = result === 'win' ? 'Bonus' : 'Lose';
  let colorText = '';
  ...
  document.getElementById('LastStage').innerText = stage;
  ...
  switch (randomNumber) {
    ...
    case 7:
    case 9:
      backgroundStyle = 'background-color: #209120 !important;'; // Green
      break;
    case 2:
    case 4:
    case 6:
  ...
  if (result === 'win') {
    ...
    document.getElementById('popupOutcome').style.display = 'block'; // Hide popupOutcome if result is win
    document.getElementById("popupOutcome").style.fontSize = "21px";
  } else {
    ...
    document.getElementById("popupOutcome").style.fontSize = "34px";
...
function showListOrderLast5(list_orders, x) {
  if (list_orders.length == 0) {
    return $(`.TimeLeft__C-num`).html(
      ``
    );
  }
  let htmls = "";
  let result = list_orders.map((list_orders) => {
    return (htmls += `
                    <div data-v-3e4c6499="" class="n${list_orders.amount}"></div>
                    `);
  });
  $(`.TimeLeft__C-num`).html(htmls);
}
    ...
    error: function (error) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (list_orders) {
            let first_five_orders = list_orders.slice(0, 5);
            console.log(first_five_orders);
            showListOrderLast5(first_five_orders,0)
            
            } else {
                console.log('list_orders is not an array');
            }
        },
      });
